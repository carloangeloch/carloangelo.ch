                                ...
                                />
                                {
                                    getScreenWidth() <= 1050 ?
                                        <div className="absolute bottom-0 w-full text-lg md:text-xl text-color-a2">
                                            <div id="detail-box" className='bg-color-d bg-opacity-80 p-1 md:p-2'>
                                                <strong className='text-xs md:text-sm xl:text-xl'>{String(work.title).length > 40 ? `${String(work.title).slice(0,40)}...` : work.title}</strong>
                                        ...
                                        >
                                            <div className="text-lg md:text-xl text-color-a bg-gradient-to-t from-white to-white/0 p-2 pt-60">
                                                ...
                                                <div className="w-full flex flex-wrap gap-1">
                                                    {work.project_type.includes("graphics") && <div className="bg-blue-700 text-color-d text-xs rounded-md p-1.5">Graphic Design</div>}
                                                    {work.project_type.includes("uiux") && <div className="bg-red-500 text-color-d text-xs rounded-md p-1.5">UI/UX</div>}
                                                    {work.project_type.includes("development") && <div className="bg-green-700 text-color-d text-xs rounded-md p-1.5">Web Development</div>}
                                                    {work.project_type.includes("ecomm") && <div className="bg-cyan-700 text-color-d text-xs rounded-md p-1.5">E-Commerce</div>}
                                                    {work.project_type.includes("3d") && <div className="bg-orange-600 text-color-d text-xs rounded-md p-1.5">3D Art</div>}
                                                    {work.project_type.includes("illustration") && <div className="bg-amber-600 text-color-d text-xs rounded-md p-1.5">Illustration</div>}
                                                    {work.project_type.includes("video") && <div className="bg-purple-700 text-color-d text-xs rounded-md p-1.5">Animation</div>}
                                                </div>
                                            </div>
                                        </motion.div>
                                }
